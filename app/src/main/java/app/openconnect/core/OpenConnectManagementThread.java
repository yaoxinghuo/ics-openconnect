...
public class OpenConnectManagementThread implements Runnable, OpenVPNManagement {
    ...
    private Set<String> getStringSetPref(final String key) {
    ...
    private void setIPInfo(VpnService.Builder b) {
        ...
        mOpenVPNService.setIPInfo(ip, mOC.getHostname(), 10000);
    }
    ...
    private void setAllowedVpnPackages(VpnService.Builder builder) {
        ...
        for (String pkg : mAllowedAppsVpn) {
            try {
                ...
                log("app no longer exists: " + pkg);
            }
        }

        if (mAllowedAppsVpnAreDisallowed) {
            log("disallowed vpn apps info: " + TextUtils.join(", ", mAllowedAppsVpn));
        } else {
